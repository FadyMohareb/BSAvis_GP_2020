...
df <- importFromGATK(
  file = "dataset2_pools_QTLseqr.tsv",
  highBulk = "pool_S3785_plus",
  lowBulk = "pool_S3785_minus",
  chromList = Chroms
)
...
pdf("C:\\Users\\s341924\\OneDrive - Cranfield University\\GP\\Plots\\plots_QTLseqR_ds2.pdf", title="S3785 pools")
for (Chrom in Chroms){
  #Plot Gprime
...
#Plot Gprime
png("plot_QTLseqr_Gprime_ds2.png", width=1300, height=500)
plotQTLStats(SNPset = df_filt, var = "Gprime", plotThreshold = TRUE, q = 0.01)
dev.off()

#Plot delta SNP-index
png("plot_QTLseqr_deltaSNP_ds2.png", width=1300, height=500)
plotQTLStats(SNPset = df_filt, var = "deltaSNP", plotIntervals = TRUE)
dev.off()

#Plot nSNPs
png("plot_QTLseqr_nSNPs_ds2.png", width=1300, height=500)
plotQTLStats(SNPset = df_filt, var = "nSNPs")
dev.off()

#Plot negLog10Pval
png("plot_QTLseqr_negLog10Pval_ds2.png", width=1300, height=500)

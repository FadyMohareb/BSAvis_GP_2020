...
#Visualise plot
pdf("plots_bsaR_deltaSNP_ds1.pdf", title="Delta(SNP-index) S3781 pools")
for (i in Chroms){
  plotWindowFreq(bsa, i, "pool_S3781_plus", "pool_S3781_minus",
                 window.size = 700000)
  mtext(paste("Delta(SNP-index) S3781 pools - Chromosome", i), outer=TRUE,  cex=1, line=-3)
}
dev.off()